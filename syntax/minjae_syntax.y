...
%token 	IDENTIFIER TYPE_IDENTIFIER FLOAT_CONSTANT INTEGER_CONSTANT
		...
		MINUSMINUS AMP BARBAR AMPAMP ARROW
...
function_definition
		...
		;
declaration
		: declaration_specifiers SEMICOLON {$$=setDeclaratorListSpecifier(makeDummyIdentifier(),$1);}
		| declaration_specifiers init_declarator_list SEMICOLON {$$=setDeclaratorListSpecifier($2,$1);}
		;
declaration_specifiers
		: type_specifier {$$=makeSpecifier($1, 0);}
		| storage_class_specifier {$$=makeSpecifier($1, 0);}
...
parameter_declaration
		...
		| declaration_specifiers
		{$$=setParameterDeclaratorSpecifier(setDeclaratorType(makeDummyIdentifier(), NIL), $1);}
		;
abstract_declarator
...
compound_statement
		...
		{checkForwardReference(); $$=makeNode(N_STMT_COMPOUND, $3, NIL, $4);}
		;
...
constant_expression
		...
		;
expression
		: assginment_expression {$$=$1;}
		;
assginment_expression
...
arg_expression_list_opt
		...
		;
arg_expression_list
		: assginment_expression {$$=makeNode(N_ARG_LIST,$1,NIL,makeNode(N_ARG_LIST_NIL,NIL,NIL,NIL));}
		| arg_expression_list COMMA assginment_expression {$$=makeNodeList(N_ARG_LIST,$1,$3);}
		;
type_name
		: declaration_specifiers {$$=setTypeNameSpecifier(NIL,$1);}
