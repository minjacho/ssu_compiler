...
void gen_declaration_list(A_ID *); void gen_declaration(A_ID *);
void gen_code_i(OPCODE,int,int); void gen_code_f(OPCODE,int,float);
void gen_code_s(OPCODE,int,char *); void gen_code_l(OPCODE,int,int);
...
void gen_expression(A_NODE *node)
{
	...
	switch(node->name) {
		...
		case N_EXP_FUNCTION_CALL :
			...
			break;
		case N_EXP_STRUCT :
			gen_error(99,node->line,"N_EXP_STRUCT");
		case N_EXP_ARROW:
			gen_expression(node->llink);
			id=node->rlink;
			if(id->address>0) {
				...
				case ID_PARM:
					switch (t->kind) {
						...
						case T_ARRAY:
							...
							break;
						case T_STRUCT:
							gen_error(99,node->line, "T_STRUCT");
							break;
						default:
...
void gen_statement(A_NODE *node, int cont_label, int break_label, A_SWITCH sw[], int *sn)
{
	...
	switch(node->name) {
		...
		case N_STMT_IF_ELSE:
			...
			break;
		case N_STMT_SWITCH:
			gen_error(99,node->line,"N_STMT_SWITCH");
			break;
		case N_STMT_WHILE:
			l3=get_label();
...
void gen_declaration(A_ID *id)
{
	...
	switch (id->kind) {
		case ID_VAR:
			if (id->init)
				gen_error(99,id->line,"INITALIZER");
			break;
		case ID_FUNC:
			if (id->type->expr) {
			...
			break;
		case ID_STRUCT:
			gen_error(99,id->line,"ID_STRUCT");
			break;
		case ID_PARM:
		case ID_TYPE:
		case ID_ENUM:
